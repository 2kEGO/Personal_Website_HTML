...
function changeTextColor(){
    ...
    if (isBlack){
        bodyContainer.classList.add('changeTextColor'); // Apply the black background
        colorButton.style.backgroundColor = 'black';
        colorButton.style.color = 'white';
    }
    else {
        bodyContainer.classList.remove('changeTextColor'); // Revert to white background
        colorButton.style.backgroundColor = 'white';
        colorButton.style.color = 'black';
    }

    // Toggle the isBlack variable
    isBlack = !isBlack;

}


// TOGGLE DISPLAY FOR GALLERY

function toggleDisplay(){
    const labels = document.querySelectorAll('.display-label');
    labels.forEach(label => {
        if (label.style.display === 'none'){
            label.style.display = 'block'; // Shows label
        } else {
